...
{
  if((contact->GetFixtureA()->GetBody()==bsensor&&contact->GetFixtureB()->GetBody()==insphere)
    ||(contact->GetFixtureA()->GetBody()==insphere&&contact->GetFixtureB()->GetBody()==bsensor))
  {
    istimeup=1;
    car2->ApplyLinearImpulse(b2Vec2(180,0), car2->GetPosition(), true);
  }

  if((contact->GetFixtureA()->GetBody()==cannonbutton&&contact->GetFixtureB()->GetBody()==insphere)
    ||(contact->GetFixtureA()->GetBody()==insphere&&contact->GetFixtureB()->GetBody()==cannonbutton))
  {
    cannonball1->ApplyLinearImpulse(b2Vec2(-5000*cosf(8.0*DEGTORAD),-5000*sinf(8.0*DEGTORAD)), cannonball1->GetPosition(), true);
    cannonball2->ApplyLinearImpulse(b2Vec2(-5000*cosf(8.0*DEGTORAD),-5000*sinf(8.0*DEGTORAD)), cannonball2->GetPosition(), true);
    cannonball3->ApplyLinearImpulse(b2Vec2(-5000*cosf(8.0*DEGTORAD),-5000*sinf(8.0*DEGTORAD)), cannonball3->GetPosition(), true);
  }
  if(((contact->GetFixtureA()->GetBody()==stopbutton&&contact->GetFixtureB()->GetBody()==mazesphere)
    ||(contact->GetFixtureA()->GetBody()==mazesphere&&contact->GetFixtureB()->GetBody()==stopbutton))&&!istimeup)
  {
    rplatform->ApplyAngularImpulse(1100.0f, 1);
    car2->ApplyLinearImpulse(b2Vec2(180,0), car2->GetPosition(), true);
  }
}